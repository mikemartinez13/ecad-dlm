...
# # LLaDA v2
# from ecad.lm_models.llada_v2.configuration_llada import LLaDAV2Config
# from ecad.lm_models.llada_v2.modeling_llada import LLaDAV2ModelLM
...
class ModelMap:
    def __init__(self, model_name:str, pretrained=None, peft=False, eagle=False):
        ...
        if lib_name == "transformers":
            if self.pretrained is None:
                if not self.peft:
                    ...
                    if "Dream" in self.model_name or "dream" in self.model_name:
                        # remove "-Custom" from the model name if it exists
                        if "-v2" in self.model_name:
                            raise ValueError("Dream v2 models are not supported yet!")
                        ...
                        raise ValueError(f"Model: {self.model_name} is not supported yet!")
                    # LLaDA variants
